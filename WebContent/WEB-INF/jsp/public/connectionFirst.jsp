<%@ page pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
<fmt:setBundle basename="messages"/>
<!DOCTYPE html>
<html data-wf-page="5bf300026add2206a70f249f"
	data-wf-site="5bf300026add22d3cd0f2499">
<head>
<meta charset="utf-8">
<title><fmt:message key="connection.title"></fmt:message></title>
<jsp:include page="/WEB-INF/jsp/head/head.jsp"></jsp:include>
</head>

<body class="body">
	<div data-w-id="f9df17d4-c9bc-cb38-975c-2b78e67e99eb"
		style="display: flex" class="loading">
		<img src="images/zolotoy_kluch-600x190.png"
			style="-webkit-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); -moz-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); -ms-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); transform-style: preserve-3d"
			alt="" class="keyimg">
	</div>
<jsp:include page="/WEB-INF/jsp/nav/nav.jsp"></jsp:include>
	<div data-w-id="9dfe1ace-3b85-782c-c65c-06aff72de6e8"
		class="imgbv w-clearfix">
		<img src="images/vetka.png"
			data-w-id="9540b10b-a521-2300-aa01-d4e07b163998" alt=""
			class="vetkaimg">
		<div  class="core">
			<div  class="txtcorew w-container">
				<p  class="coretxtw"><fmt:message key="connection.shortintroductionfirst"></fmt:message></p>
				<p  class="coretxtw"><fmt:message key="connection.shortintroductionsecond"></fmt:message></p>
				<p  class="coretxtw"><fmt:message key="connection.shortintroductionthird"></fmt:message></p>
				<p  class="coretxtw"><fmt:message key="connection.shortintroductionfourth"></fmt:message></p>
			</div>
			<img src="images/zolotoy_kluch-600x190.png"
				data-w-id="48c864dc-1ffc-9e3f-266a-880be2f1928f"
				style="-webkit-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); -moz-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); -ms-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0DEG) rotateY(0) rotateZ(0) skew(0, 0); transform-style: preserve-3d"
				alt="" class="keyimg">
			<div class="butformulairenreg">
			
				<a  href="<c:url value="/connection"></c:url>"
							class="buttonstandart"><fmt:message key="connection.meconnecter"></fmt:message></a>
					
				<a  href="<c:url value="/inscription"></c:url>"
							class="buttonstandart"><fmt:message key="connection.creercompte"></fmt:message></a>

			</div>
		</div>
		<div id="imga" data-w-id="85ccf075-60b1-9d4a-a4e0-38751406c605"
			style="opacity: 1; display: block; -webkit-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -moz-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -ms-transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); transform: translate3d(0, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); transform-style: preserve-3d"
			class="imgbva">
			<div data-w-id="76fd4d4c-ffe6-64d4-34b9-2931c6ba6bf9"
				style="display: block; opacity: 1" class="couverbva"></div>
			<img src="images/bvimga.png"
				data-w-id="d8dad0bb-29f7-67e4-6b7a-94511160a198"
				style="-webkit-transform: translate3d(0, 0, 0) scale3d(1.4, 1.4, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -moz-transform: translate3d(0, 0, 0) scale3d(1.4, 1.4, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -ms-transform: translate3d(0, 0, 0) scale3d(1.4, 1.4, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); transform: translate3d(0, 0, 0) scale3d(1.4, 1.4, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0)"
				alt="" class="rose">
			<div class="blockimgbva w-clearfix">
				<div data-w-id="60b799b7-8fe8-5559-ee27-7d55f32209ef"
					style="-webkit-transform: translate3d(-600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -moz-transform: translate3d(-600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -ms-transform: translate3d(-600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); transform: translate3d(-600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); opacity: 0"
					class="traitl"></div>
				<img src="images/glassdel.png"
					data-w-id="2b0bf9ed-454b-8d8c-aac2-05161fa1e3f0" style="opacity: 0"
					alt="" class="imgglassdel">
				<div data-w-id="446ccad7-1f4a-5d8b-164d-497994df383e"
					style="-webkit-transform: translate3d(600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -moz-transform: translate3d(600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); -ms-transform: translate3d(600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); transform: translate3d(600PX, 0, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0); opacity: 0"
					class="traitr"></div>
			</div>
			<div data-w-id="01135ae9-26ce-0904-1c3a-f9132c53fe28"
				class="blocktxtbv">
				<h1 data-w-id="cf388477-460b-0c8e-8d99-efa38948d776"
					class="headingbvbb">
					<span data-w-id="cf388477-460b-0c8e-8d99-efa38948d777"
						class="textbva"><fmt:message key="connection.bvfirst"></fmt:message></span> <br>
				</h1>
				<h1 data-w-id="ba171d92-f7cc-acbe-abf0-39010432a838"
					class="headingbvc">
					<span class="textbva"><fmt:message key="connection.bvsecond"></fmt:message></span>
				</h1>
			</div>
		</div>
	</div>
	<script
		src="https://d1tdp7z6w94jbb.cloudfront.net/js/jquery-3.3.1.min.js"
		type="text/javascript"
		integrity="sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8="
		crossorigin="anonymous"></script>
	<script src="js/jscave.js" type="text/javascript"></script>
	<!-- [if lte IE 9]><script src="https://cdnjs.cloudflare.com/ajax/libs/placeholders/3.0.2/placeholders.min.js"></script><![endif] -->
</body>
</html>